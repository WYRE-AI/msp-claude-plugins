...
- The org's Salesbuildr credential is stored once at the gateway, so
  ...
  form, which overwrites the stored credential in place, and nothing tracks
...
- Removing someone from the organisation clears their per-vendor grants and
  ...
  hour, but it reaches only a personal Salesbuildr connection made with their
