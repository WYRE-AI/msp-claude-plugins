...
description: >
  ...
  authentication, the single production base URL, the full 20-tool MCP
...
|------|-----------|-------------|
| `axcient_test_connection` | *(none)* | Server's Axcient connection/credential state |
| `axcient_get_organization` | *(none)* | The caller's own organization, resolved from the API key |

### Clients

| Tool | Arguments | Description |
|------|-----------|-------------|
| `axcient_list_clients` | `include_appliances?` | Every client visible to this credential |
| `axcient_get_client` | `client_id`, `include_appliances?` | One client, with health status and protected-system counters |
| `axcient_get_d2c_agent_token` | `client_id`, `vault_id` | Mints a direct-to-cloud agent enrollment token (POST, 201) |

### Devices

| Tool | Arguments | Description |
|------|-----------|-------------|
| `axcient_list_devices` | `limit?`, `offset?` | Every device across the organization |
| `axcient_list_devices_by_client` | `client_id`, `service_id?`, `d2c_only?` | Devices for one client |
| `axcient_get_device` | `device_id` | Full detail for one device |
| `axcient_get_device_autoverify` | `device_id` | Latest screenshot-verification (AutoVerify) results |
| `axcient_get_device_restore_points` | `device_id` | Available restore points for the device |

### Jobs

| Tool | Arguments | Description |
|------|-----------|-------------|
| `axcient_list_jobs_by_device` | `client_id`, `device_id` | All backup jobs for a device |
| `axcient_get_job` | `client_id`, `device_id`, `job_id` | One job |
| `axcient_get_job_history` | `client_id`, `device_id`, `job_id`, `limit?`, `offset?`, `starttime_begin?` | Run history for a job |
...
| Tool | Arguments | Description |
|------|-----------|-------------|
| `axcient_list_vaults` | `vault_type?` (`Private`\|`Cloud`), `active?`, `with_url?`, `limit?`, `include_devices?` | Every vault (private on-prem or Axcient cloud) |
| `axcient_get_vault` | `vault_id` | One vault |
| `axcient_get_vault_threshold` | `vault_id` | Current connectivity-loss alert threshold |
| `axcient_set_vault_threshold` | `vault_id`, `threshold` | **Changes** the connectivity-loss threshold (minutes) |

### Appliances

| Tool | Arguments | Description |
|------|-----------|-------------|
| `axcient_list_appliances` | `service_id?`, `include_devices?` | Every appliance in the organization |
| `axcient_list_appliances_by_client` | `client_id`, `include_devices?` | Appliances for one client |
| `axcient_get_appliance` | `appliance_id`, `include_devices?` | One appliance |
...
- Treat `axcient_get_device_restore_points` and `axcient_get_job_history`
...
- `axcient_get_d2c_agent_token` returns short-lived enrollment
...
- Cache `axcient_list_clients` and `axcient_list_appliances` results within
