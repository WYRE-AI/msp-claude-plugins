...
Why the destructive tier is drawn by consequence rather than verb:
...
- Write tools: agent drafts the exact call, human approves, then it runs.
  Note that creating an invoice is write-tier only because it stays
  internal until sent — `qbo_invoices_create` followed by
  `qbo_invoices_send` is a destructive sequence.
- Destructive tools: require a named human approver per invocation, and
  ...
  transaction anyway. Do not grant these to scheduled or unattended
  agents.
- **Never grant any write or destructive tool against a production realm
  during a close period** unless the approver is the person doing the
  close.
...
- **Invoices and customers have no update tool here.** `qbo_invoices_get`
  ...
  entry — that is the destructive tier, reached sideways.
