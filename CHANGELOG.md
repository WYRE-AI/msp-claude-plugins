...
- Docs: **Advanced Workflows batch 2** — seven new scheduled-routine guides picked from the Agent → Routine Catalog backlog, spanning connectors not covered by batch 1. Grounded against live gateway connectors: **Contract Renewal Tracker** (Autotask — weekly expiring-contract/MRR-at-risk pipeline), **Service Desk SLA Triage** (HaloPSA — hourly SLA-breach triage, idempotent via a per-ticket triage note), **Cash Flow Analyzer** (QuickBooks Online, with Xero documented as a connector swap), and **Client Profitability Reporter** (QuickBooks Online — monthly revenue-ranked client report with company net-margin context; true cost-allocated margin lives in the `service-profitability-auditor` portfolio agent). Documented but flagged *pending* until their connectors are live: **Pax8 Renewal & License Optimizer** (Pax8 surfaces no tools through the gateway yet), **Sales Pipeline Pulse** (HubSpot not connected), and **Pending-Proposal Tracker** (PandaDoc not connected). The Agent → Routine Catalog rows for all of these are annotated accordingly
- `wyre-gateway` portfolio plugin: **11 new multi-vendor portfolio agents** (v1.1.1 → 1.2.0), each a single-prompt workflow spanning five to ten connected systems: `offboarding-orchestrator`, `user-lifecycle-orchestrator`, `asset-reconciliation-auditor`, `license-true-up-reconciler`, `service-profitability-auditor`, `portfolio-threat-sweep`, `vulnerability-remediation-prioritizer`, `change-drift-sentinel`, `dr-readiness-auditor`, `book-of-business-pulse`, and `ticket-deflection-analyzer`. Marketplace version bumped to 1.8.0
