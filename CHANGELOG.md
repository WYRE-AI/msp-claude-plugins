...
- Docs: **Advanced Workflows batch 2** — four new scheduled-routine guides picked from the Agent → Routine Catalog backlog, spanning connectors not covered by batch 1: **Contract Renewal Tracker** (Autotask — weekly expiring-contract/MRR-at-risk pipeline), **Service Desk SLA Triage** (HaloPSA — hourly SLA-breach triage, idempotent via a per-ticket triage note), **Cash Flow Analyzer** (built on the live QuickBooks Online connector, with Xero documented as a connector swap), and **Pax8 Renewal & License Optimizer** (documented but flagged *pending* — the Pax8 connector surfaces no tools through the gateway yet). The Agent → Routine Catalog rows for these agents are annotated accordingly
